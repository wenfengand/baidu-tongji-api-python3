...
def gzdecode(data):
    ...
    '''
    f = StringIO(data)
    gziper = GzipFile(fileobj=f, compresslevel=9)
...
class BaiduTongji(object):
    ...
    def dologin(self):
        ...
        # post
        resp = requests.post(LOGIN_URL, data=post_data, headers=headers)
        ret_json = gzdecode(resp.content[8:])

        if len(ret_json) > 0:
            ret = json.loads(ret_json)
            if ret['retcode'] == 0:
                print('dologin:', ret['retmsg'])
                print(ret['ucid'])
                print(ret['st'])
        else:
            ret = None 
...
if __name__ == '__main__':
    # 配置自己的用户/密码/token
    bdtj = BaiduTongji(secret.user_name, secret.user_passwd, secret.token)
